...
本プロジェクトは、制限された視野を移動して行う視覚探索場面において、視覚的ワーキングメモリおよび物体の配置構造が探索効率に与える影響を解明することを目的とした実験プログラムです。jsPsych v7を使用してブラウザ上で実行される心理学実験として実装されています。
...
1. リポジトリをクローン：
```bash
git clone https://github.com/YuseiNozaki/wm-itemstructure-effect-2.git
cd wm-itemstructure-effect-2
```

2. ローカルサーバーを起動（例：Pythonを使用）：
```bash
# Python 3の場合
...
3. ブラウザで `http://localhost:8000/index.html` にアクセス

### オンライン公開

- GitHub Pagesやその他のホスティングサービスにindex.htmlをアップロードするだけで実行可能
- CDNからjsPsychライブラリを読み込んでいるため、追加のビルド手順は不要
...
実験終了時に、jsPsychのデータ出力機能により実験データが画面に表示されます。このデータには以下の情報が含まれます：
...
本実験プログラムは認知心理学研究のために開発されました。jsPsychコミュニティに感謝いたします。

## 更新履歴

- 2025年: 初版リリース
  - jsPsych v7への移行
  - フルスクリーン・ウィンドウ・スクロール条件の実装
  - 記憶負荷条件の追加
  - カウンターバランシング機能の実装